...
class ReservationControllerSecurityTest {
 ...
 void normalUserCannotGetAllReservations() throws Exception {
  mockMvc.perform(get("/api/reservations").with(authenticationFor("user01", MemberRole.USER))).andExpect(status().isForbidden());
 }
 ...
 void adminCanGetAllReservations() throws Exception {
  ...
  mockMvc.perform(get("/api/reservations").with(authenticationFor("admin01", MemberRole.ADMIN))).andExpect(status().isOk());
  verify(reservationService).getAllReservations();
 }

 @Test
 void normalUserCannotPatchReservationStatus() throws Exception {
  mockMvc.perform(patch("/api/reservations/1/status").with(authenticationFor("user01", MemberRole.USER)).with(csrf()).contentType(MediaType.APPLICATION_JSON).content("{\"status\":\"COMPLETED\"}")).andExpect(status().isForbidden());
 ...
 void adminCanPatchReservationStatus() throws Exception {
  when(reservationService.updateReservationStatus(eq(1), any(ReservationStatusUpdateRequest.class))).thenReturn(sampleReservation());
  mockMvc.perform(patch("/api/reservations/1/status").with(authenticationFor("admin01", MemberRole.ADMIN)).with(csrf()).contentType(MediaType.APPLICATION_JSON).content("{\"status\":\"COMPLETED\"}")).andExpect(status().isOk());
 ...
 private ReservationResponse sampleReservation() {
  return ReservationResponse.builder().reservationId(1).memberId("user01").designerId(1).designerName("Designer").salonServiceId(1).serviceName("Cut").reservationDate(LocalDate.of(2026, 4, 8)).reservationTime(LocalTime.of(10, 0)).createdAt(LocalDateTime.of(2026, 4, 8, 9, 0)).build();
 }
