...
class ReservationDummySeederTest {
    ...
    void seedReservationsCreatesReservationsUpToTargetCount() {
        ...
        assertThat(reservationCaptor.getAllValues()).extracting(Reservation::getStatus)
                .containsExactly(
        ...
        assertThat(reservationCaptor.getAllValues()).extracting(Reservation::getTotalPrice)
                .allSatisfy(price -> assertThat((Integer) price).isGreaterThanOrEqualTo(30000));
    ...
    @Test
    void seedReservationsSkipsOccupiedSlotAndFindsNextAvailableSlot() {
        ReflectionTestUtils.setField(reservationDummySeeder, "targetReservationCount", 1);

        Salon salon = Salon.builder().salonId(1).name("Test Salon").build();
        Designer designer = Designer.builder().designerId(1).salon(salon).name("Senior Director").careerYears(5).build();
        SalonService service = SalonService.builder().serviceId(1).salon(salon).name("Basic Cut").price(30000).duration(60).build();
        Member user = Member.builder().memberId("user1").name("User1").role(MemberRole.USER).status(MemberStatus.ACTIVE).build();

        when(reservationRepository.count()).thenReturn(0L);
        when(memberRepository.findByRoleAndStatusOrderByCreatedAtAsc(MemberRole.USER, MemberStatus.ACTIVE)).thenReturn(List.of(user));
        when(designerRepository.findAll()).thenReturn(List.of(designer));
        when(salonServiceRepository.findAll()).thenReturn(List.of(service));
        when(reservationSlotRepository.existsByDesigner_DesignerIdAndReservationDateAndSlotTime(eq(1), any(), any()))
        ...
        verify(reservationSlotRepository, times(2))
                .existsByDesigner_DesignerIdAndReservationDateAndSlotTime(eq(1), any(), any());
        verify(reservationRepository, times(1)).save(any(Reservation.class));
        verify(reservationSlotRepository, times(1)).save(any(ReservationSlot.class));
    }
}
