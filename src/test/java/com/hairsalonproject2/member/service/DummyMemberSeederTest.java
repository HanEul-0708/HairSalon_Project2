...
class DummyMemberSeederTest {
 ...
 void runSeedsMissingMembersUpToTargetCounts() throws Exception {
  ...
  assertThat(memberCaptor.getAllValues()).extracting(Member::getMemberId).contains("designer1", "designer2", "user1", "user2");
  assertThat(memberCaptor.getAllValues()).extracting(Member::getName).contains("디자이너1", "디자이너2", "회원1", "회원2");
 }
