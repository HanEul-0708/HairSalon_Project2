...
class MemberFlowMvcTest {
 ...
 void signupMyPageAndProfileEditFlow() throws Exception {
  when(memberService.getMyDetail("user01")).thenReturn(MemberDetailResponse.builder().memberId("user01").name("Tester").phone("010-1234-5678").email("tester@example.com").role(MemberRole.USER).status(MemberStatus.ACTIVE).createdAt(LocalDateTime.of(2026, 4, 8, 9, 0)).build());
  ...
  mockMvc.perform(get("/members/me").with(authenticationFor("user01", MemberRole.USER))).andExpect(status().isOk()).andExpect(view().name("member/mypage"));
  mockMvc.perform(post("/members/me/edit").with(authenticationFor("user01", MemberRole.USER)).with(csrf()).param("name", "Edited User").param("phone", "010-9999-9999").param("email", "edited@example.com")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/members/me"));
