...
class DesignerAiRecommendationServiceTest {
 ...
 @BeforeEach
 void setUp() {
  service = new DesignerAiRecommendationService(designerRepository, salonServiceRepository, reviewRepository, designerAiClient);
 }
 ...
 void recommendsDesignerByStyleKeywordWithoutLlm() {
  ...
  when(designerRepository.findDesignerRatingRows()).thenReturn(List.of(ratingRow(1, 4.7, 18L), ratingRow(2, 4.9, 12L)));
  when(salonServiceRepository.findBySalonSalonId(1)).thenReturn(List.of(service(colorSalon, "프리미엄 염색", "톤다운, 톤업, 브릿지 컬러")));
  when(salonServiceRepository.findBySalonSalonId(2)).thenReturn(List.of(service(cutSalon, "남성 커트", "깔끔한 비즈니스 커트")));
 ...
 void appliesLlmOrderingAndReasonWhenAvailable() {
  ...
  when(designerRepository.findDesignerRatingRows()).thenReturn(List.of(ratingRow(1, 4.8, 10L), ratingRow(2, 4.5, 8L)));
