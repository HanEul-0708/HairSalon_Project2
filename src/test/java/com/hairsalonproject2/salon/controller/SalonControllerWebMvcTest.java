...
class SalonControllerWebMvcTest {
 ...
 void setUp() {
  ...
  mockMvc = MockMvcBuilders.standaloneSetup(salonController, serviceController, designerController, adminDesignerController).setControllerAdvice(new BDomainPageExceptionHandler(), new GlobalPageExceptionHandler()).addDispatcherServletCustomizer(dispatcherServlet -> dispatcherServlet.setThrowExceptionIfNoHandlerFound(true)).setViewResolvers(viewResolver).build();
 }

 @Test
 void anonymousCannotOpenSalonCreateForm() throws Exception {
  mockMvc.perform(get("/salons/new")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/members/login"));
 }

 @Test
 void salonCreateFormOpensForAdmin() throws Exception {
  mockMvc.perform(get("/salons/new").principal(new UsernamePasswordAuthenticationToken("admin", "n/a", List.of(new SimpleGrantedAuthority("ROLE_ADMIN"))))).andExpect(status().isOk()).andExpect(view().name("salon/form"));
 }
 ...
 void designerListCanonicalizesLikesSortQuery() throws Exception {
  mockMvc.perform(get("/designers").param("sortBy", "likes")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/designers"));
 }

 @Test
 void designerListCanonicalizesNewestSortQuery() throws Exception {
  mockMvc.perform(get("/designers").param("sortBy", "newest")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/designers"));
 }

 @Test
 void designerRankingRouteIsRemoved() throws Exception {
  mockMvc.perform(get("/designers/ranking")).andExpect(status().isNotFound());
 }
 ...
 void popularServiceRouteRedirectsToSortedList() throws Exception {
  mockMvc.perform(get("/salon-services/popular")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salon-services?sortBy=rating"));
 }

 @Test
 void trendServiceRouteRedirectsToTrendList() throws Exception {
  mockMvc.perform(get("/salon-services/trend")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salon-services?sortBy=trend"));
 }

 @Test
 void adminDesignerCreateFormUsesDesignerTemplate() throws Exception {
  mockMvc.perform(get("/admin/designers/new")).andExpect(status().isOk()).andExpect(view().name("designer/form"));
 }
 ...
 void missingSalonReturnsNotFoundStatus() throws Exception {
  ...
  mockMvc.perform(get("/salons/999999")).andExpect(status().isNotFound()).andExpect(view().name("error/common-error"));
 }

 @Test
 void priceCompareRouteRedirectsToSalonServiceCompare() throws Exception {
  mockMvc.perform(get("/salons/price-compare").param("serviceName", "cut").param("region", "seoul")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salon-services/compare?serviceName=cut&region=seoul"));
 }

 @Test
 void salonListPageLoadsWithMergedFeatures() throws Exception {
  mockMvc.perform(get("/salons")).andExpect(status().isOk()).andExpect(view().name("salon/list")).andExpect(model().attributeExists("salons")).andExpect(model().attributeExists("search")).andExpect(model().attributeExists("branchMarkers")).andExpect(model().attribute("pageTitle", "살롱 통합 탐색")).andExpect(model().attribute("activePreset", "all")).andExpect(model().attribute("branchView", true)).andExpect(model().attribute("showKakaoSyncAction", true)).andExpect(model().attribute("searchAction", "/salons")).andExpect(model().attribute("primaryKeywordPlaceholder", "이름, 주소, 설명")).andExpect(model().attribute("branchMapStatus", "카카오 REST API 키가 설정되지 않아 지도를 표시할 수 없습니다.")).andExpect(model().attribute("kakaoResultsMessage", "검색어를 입력하면 내부 살롱과 카카오 외부 결과를 함께 비교할 수 있습니다."));
 }

 @Test
 void salonSearchAliasRedirectsToUnifiedSalonPage() throws Exception {
  mockMvc.perform(get("/salons/search").param("keyword", "cut")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salons?keyword=cut"));
 }

 @Test
 void topRatedRouteRedirectsToSalonPreset() throws Exception {
  mockMvc.perform(get("/salons/top-rated")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salons?preset=top-rated&minRating=4.0&sort=rating"));
 }

 @Test
 void topRated45PresetLoadsOnUnifiedSalonPage() throws Exception {
  mockMvc.perform(get("/salons").param("preset", "top-rated-4-5")).andExpect(status().isOk()).andExpect(view().name("salon/list")).andExpect(model().attribute("activePreset", "top-rated-4-5"));
 }

 @Test
 void recommendRouteRedirectsToSalonPreset() throws Exception {
  mockMvc.perform(get("/salons/recommend-by-service").param("styleKeyword", "perm")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salons?keyword=perm&preset=recommend-by-service&sort=rating"));
 }

 @Test
 void branchesViewRedirectsToCanonicalSalonPage() throws Exception {
  mockMvc.perform(get("/salons").param("view", "branches").param("keyword", "cut")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salons?keyword=cut"));
 }

 @Test
 void kakaoViewRedirectsToCanonicalSalonPage() throws Exception {
  mockMvc.perform(get("/salons").param("view", "kakao").param("keyword", "cut")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salons?keyword=cut"));
 }

 @Test
 void legacyBranchesRouteRedirectsToUnifiedSalonPage() throws Exception {
  mockMvc.perform(get("/salons/branches")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salons"));
 }

 @Test
 void kakaoSearchMessageShownOnMergedPageWhenKeyMissing() throws Exception {
  mockMvc.perform(get("/salons").param("keyword", "cut")).andExpect(status().isOk()).andExpect(view().name("salon/list")).andExpect(model().attribute("pageTitle", "살롱 통합 탐색")).andExpect(model().attribute("searchAction", "/salons")).andExpect(model().attribute("branchMapStatus", "카카오 REST API 키가 설정되지 않아 지도를 표시할 수 없습니다.")).andExpect(model().attribute("kakaoResultsMessage", "카카오 API 키가 설정되지 않아 외부 검색을 수행할 수 없습니다."));
 }

 @Test
 void legacyKakaoRouteRedirectsToUnifiedSalonPage() throws Exception {
  mockMvc.perform(get("/salons/kakao-search").param("keyword", "cut")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/salons?keyword=cut"));
 }
}
