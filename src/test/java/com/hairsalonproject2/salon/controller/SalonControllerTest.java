...
class SalonControllerTest {
    ...
    void listPopulatesUnifiedSalonPageWhenKakaoRestKeyMissing() {
        ...
        when(salonQueryService.search(request)).thenReturn(List.of(
        ...
        assertThat(model.getAttribute("branchMapStatus"))
                .isEqualTo("카카오 REST API 키가 설정되지 않아 지도를 표시할 수 없습니다.");
        assertThat(model.getAttribute("kakaoResultsMessage"))
                .isEqualTo("카카오 API 키가 설정되지 않아 외부 검색을 수행할 수 없습니다.");
    ...
    void listBuildsBranchMarkersFromAddressSearch() {
        ...
        when(kakaoLocalSearchClient.searchAddress("서울 강남구 테헤란로 123"))
                .thenReturn(Optional.of(KakaoAddressSearchResult.builder()
                        .longitude(new BigDecimal("127.031393491745"))
                        .latitude(new BigDecimal("37.4995539438207"))
                        .build()));

        salonController.list(null, null, null, new LinkedMultiValueMap<>(), request, model);

        @SuppressWarnings("unchecked")
        List<SalonBranchMarkerResponse> markers = (List<SalonBranchMarkerResponse>) model.getAttribute("branchMarkers");
