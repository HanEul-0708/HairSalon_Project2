...
class SalonSeedDataFactoryTest {
 ...
 void createDesignersGeneratesCareerSpecialtyBasedProfiles() {
  Salon salon = Salon.builder().externalId("kakao-100").name("시드헤어").address("서울 강남").build();
  List<Designer> designers = factory.createDesigners(salon);
  assertThat(designers).hasSize(3);
  assertThat(designers).extracting(Designer::getName).containsExactly(designers.get(0).getName(), designers.get(1).getName(), designers.get(2).getName());
 ...
 void createServicesUsesRuleBasedPricing() {
  Salon salon = Salon.builder().externalId("kakao-101").name("시드헤어").address("서울 강남").build();
  List<SalonService> services = factory.createServices(salon);
  assertThat(services).hasSize(5);
  assertThat(services).extracting(SalonService::getName).containsExactly("여성 커트", "남성 커트", "염색", "펌", "스타일링");
