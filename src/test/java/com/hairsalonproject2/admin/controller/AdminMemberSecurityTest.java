...
class AdminMemberSecurityTest {
 ...
 void anonymousUserIsRedirectedToLoginForAdminPage() throws Exception {
  mockMvc.perform(get("/admin/members")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrlPattern("**/members/login"));
 }

 @Test
 @WithMockUser(roles = "USER")
 void normalUserCannotAccessAdminPage() throws Exception {
  mockMvc.perform(get("/admin/members")).andExpect(status().isForbidden());
 }

 @Test
 @WithMockUser(roles = "ADMIN")
 void adminPostWithoutCsrfIsForbidden() throws Exception {
  mockMvc.perform(post("/admin/members/user01/role").param("role", "USER")).andExpect(status().isForbidden());
 ...
 void adminPostWithCsrfIsAllowed() throws Exception {
  mockMvc.perform(post("/admin/members/user01/role").with(authenticationFor("admin01", MemberRole.ADMIN)).with(csrf()).param("role", "USER")).andExpect(status().is3xxRedirection());
 }

 @Test
 @WithMockUser(roles = "ADMIN")
 void adminCanFilterMemberListByStatus() throws Exception {
  when(memberService.searchMembers(eq(null), eq(null), eq(MemberStatus.INACTIVE), eq("latest"), eq(0))).thenReturn(new PageImpl<>(List.of(MemberSummaryResponse.builder().memberId("user01").name("Tester").phone("010-1234-5678").email("tester@example.com").role(MemberRole.USER).status(MemberStatus.INACTIVE).createdAt(LocalDateTime.of(2026, 4, 6, 10, 0)).build())));
  mockMvc.perform(get("/admin/members").param("status", "INACTIVE")).andExpect(status().isOk()).andExpect(model().attributeExists("selectedStatus"));
 }

 private RequestPostProcessor authenticationFor(String memberId, MemberRole role) {
  Member member = Member.builder().memberId(memberId).password("encoded-password").name("Admin").phone("010-1234-5678").email(memberId + "@example.com").role(role).status(MemberStatus.ACTIVE).build();
