  ...
  for (ImageMeta imageMeta : contentImages) {
   if (!currentImageMap.containsKey(imageMeta.imageUrl())) {
	...
	if (savedName != null && !savedName.isBlank()) {
	 BoardImage boardImage = BoardImage.builder().board(board).originalName(resolveOriginalImageName(imageMeta, savedName)).savedName(savedName).filePath(uploadPath).imageUrl(imageMeta.imageUrl()).build();
