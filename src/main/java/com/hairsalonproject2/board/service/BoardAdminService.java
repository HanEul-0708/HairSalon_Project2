...
public class BoardAdminService {
 ...
 private Board findBoard(Integer boardId) {
  return boardRepository.findById(boardId).orElseThrow(() -> new BoardException("존재하지 않는 게시글입니다."));
 }

 private Member findMember(String memberId) {
  return memberRepository.findById(memberId).orElseThrow(() -> new BoardException("존재하지 않는 회원입니다."));
 }
 ...
 private void saveDeleteLog(Board board, String adminId, String reason) {
  BoardDeleteLog log = BoardDeleteLog.builder().boardId(board.getBoardId()).boardType(board.getType()).boardTitle(board.getTitle()).writerId(board.getMember().getMemberId()).deletedBy(adminId).deleteReason(reason).build();
