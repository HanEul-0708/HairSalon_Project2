...
public class BoardController {
 ...
 @GetMapping("/notices")
 public String noticeList(BoardSearchRequest request, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, Model model) {
 ...
 public String noticeDetail(@PathVariable Integer boardId, HttpServletRequest request, HttpServletResponse response, Model model) {
 ...
 @GetMapping("/qna")
 public String qnaList(BoardSearchRequest request, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, Model model) {
 ...
 public String qnaDetail(@PathVariable Integer boardId, HttpServletRequest request, HttpServletResponse response, Model model) {
 ...
 @PostMapping("/qna")
 @PreAuthorize("isAuthenticated()")
 public String qnaCreate(@Valid BoardCreateRequest request, BindingResult bindingResult, @RequestParam(value = "files", required = false) List<MultipartFile> files, @AuthenticationPrincipal UserDetails userDetails, Model model) {
 ...
 @PostMapping("/notices")
 @PreAuthorize("hasRole('ADMIN')")
 public String noticeCreate(@Valid BoardCreateRequest request, BindingResult bindingResult, @RequestParam(value = "files", required = false) List<MultipartFile> files, @AuthenticationPrincipal UserDetails userDetails, Model model) {
  if (bindingResult.hasErrors()) {
   model.addAttribute("boardType", BoardType.NOTICE.name());
   return "board/write";
  }
 ...
 @PreAuthorize("isAuthenticated()")
 public String edit(@PathVariable Integer boardId, @Valid BoardUpdateRequest request, BindingResult bindingResult, @RequestParam(value = "files", required = false) List<MultipartFile> files, @AuthenticationPrincipal UserDetails userDetails, Model model) {
  BoardType boardType = boardService.getBoardType(boardId);
  if (bindingResult.hasErrors()) {
   BoardDetailResponse board = boardService.getDetailOnly(boardId);
   model.addAttribute("board", board);
 ...
 @PreAuthorize("isAuthenticated()")
 public String delete(@PathVariable Integer boardId, @AuthenticationPrincipal UserDetails userDetails) {
 ...
 private void applyBoardPageModel(Model model, Page<BoardResponse> boardPage, String keyword, BoardType boardType, int size) {
 ...
 private String buildDetailPath(Integer boardId, BoardType boardType) {
  return boardType == BoardType.NOTICE ? "/boards/notices/" + boardId : "/boards/qna/" + boardId;
 }
