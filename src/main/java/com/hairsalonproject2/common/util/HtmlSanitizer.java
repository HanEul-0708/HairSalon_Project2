...
public final class HtmlSanitizer {
 ...
 private static final Set<String> ALLOWED_IMAGE_CLASSES = Set.of("board-image-align-left", "board-image-align-center", "board-image-align-right");

 private static final Set<String> ALLOWED_STYLE_PROPERTIES = Set.of("color", "background-color", "background", "font-size", "text-align");

 private static final Pattern SAFE_COLOR = Pattern.compile("^(#[0-9a-fA-F]{3,8}|rgb(a)?\\([0-9\\s.,%]+\\)|hsl(a)?\\([0-9\\s.,%]+\\)|[a-zA-Z]+)$");

 private static final Pattern SAFE_FONT_SIZE = Pattern.compile("^[0-9]+(\\.[0-9]+)?(px|pt|em|rem|%)$");

 private static final Set<String> SAFE_FONT_SIZE_KEYWORDS = Set.of("xx-small", "x-small", "small", "medium", "large", "x-large", "xx-large", "smaller", "larger");
 ...
 private static String sanitizeInlineStyles(String originalHtml, String cleanedHtml) {
  ...
  for (Element element : document.body().select("[style]")) {
   String sanitizedStyle = Arrays.stream(element.attr("style").split(";")).map(String::trim).filter(style -> !style.isBlank()).map(HtmlSanitizer::sanitizeStyleDeclaration).filter(style -> style != null && !style.isBlank()).reduce((left, right) -> left + "; " + right).orElse("");
 ...
 private static boolean isSafeStyleValue(String property, String value) {
  return switch (property) {
   ...
   case "font-size" ->
		   SAFE_FONT_SIZE.matcher(value).matches() || SAFE_FONT_SIZE_KEYWORDS.contains(value.toLowerCase(Locale.ROOT));
   case "text-align" -> Set.of("left", "center", "right", "justify").contains(value.toLowerCase(Locale.ROOT));
   default -> false;
  };
 }
}
