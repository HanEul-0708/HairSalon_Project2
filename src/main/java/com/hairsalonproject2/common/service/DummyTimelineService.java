...
public class DummyTimelineService {
 ...
 @Transactional
 public void alignSalonSeedTimeline(Integer salonId, List<Integer> designerIds, List<Integer> serviceIds) {
  if (salonId == null) {
   return;
  }
  LocalDateTime salonCreatedAt = LocalDate.now().minusDays(45L + (salonId % 20)).atTime(9, 0).plusMinutes((salonId % 6) * 10L);
 ...
 @Transactional
 public void alignReservationTimeline(Integer reservationId, Integer slotId, LocalDate reservationDate, LocalTime reservationTime, LocalDateTime minimumCreatedAt) {
  ...
  LocalDateTime reservationCreatedAt = desiredReservationCreatedAt.isAfter(minimumCreatedAt) ? desiredReservationCreatedAt : minimumCreatedAt.plusHours(2);
 ...
 @Transactional
 public void alignReviewTimeline(Integer reviewId, Integer reservationId, LocalDate reservationDate, LocalTime reservationTime) {
  if (reviewId == null || reservationId == null || reservationDate == null || reservationTime == null) {
   return;
  }
  LocalDateTime reservationCreatedAt = jdbcTemplate.queryForObject("select created_at from reservation where reservation_id = ?", (rs, rowNum) -> rs.getTimestamp(1).toLocalDateTime(), reservationId);
  if (reservationCreatedAt == null) {
   return;
  }
  LocalDateTime desiredReviewCreatedAt = reservationDate.atTime(reservationTime).plusHours(2);
  LocalDateTime reviewCreatedAt = desiredReviewCreatedAt.isAfter(reservationCreatedAt) ? desiredReviewCreatedAt : reservationCreatedAt.plusHours(1);
 ...
 private void updateTimeTable(String tableName, String idColumn, Integer id, LocalDateTime createdAt) {
  jdbcTemplate.update("update " + tableName + " set created_at = ?, updated_at = ? where " + idColumn + " = ?", Timestamp.valueOf(createdAt), Timestamp.valueOf(createdAt), id);
 }

 private void updateCreatedTable(String tableName, String idColumn, Integer id, LocalDateTime createdAt) {
  jdbcTemplate.update("update " + tableName + " set created_at = ? where " + idColumn + " = ?", Timestamp.valueOf(createdAt), id);
 }
}
