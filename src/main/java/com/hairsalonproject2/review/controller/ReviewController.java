...
public class ReviewController {
 ...
 @PostMapping
 public ReviewResponse createReview(@AuthenticationPrincipal CustomUserDetails userDetails, @Valid @RequestBody ReviewCreateRequest request) {
  return reviewService.createReview(userDetails.getMember().getMemberId(), request);
 }

 @GetMapping("/{reviewId}")
 public ReviewResponse getReview(@PathVariable Integer reviewId, @AuthenticationPrincipal CustomUserDetails userDetails, HttpServletRequest request) {
  return reviewService.getReview(reviewId, getLoginMemberId(userDetails), getVisitorToken(request));
 }

 @GetMapping
 public List<ReviewResponse> getAllReviews(@AuthenticationPrincipal CustomUserDetails userDetails, HttpServletRequest request, @RequestParam(required = false) Integer designerId, @RequestParam(defaultValue = "latest") String sortBy) {
  return reviewService.getAllReviews(getLoginMemberId(userDetails), getVisitorToken(request), designerId, sortBy);
 }

 @GetMapping("/member/{memberId}")
 public List<ReviewResponse> getReviewsByMember(@PathVariable String memberId, @AuthenticationPrincipal CustomUserDetails userDetails, HttpServletRequest request) {
  return reviewService.getReviewsByMember(memberId, getLoginMemberId(userDetails), getVisitorToken(request));
 }

 @PutMapping("/{reviewId}")
 public ReviewResponse updateReview(@AuthenticationPrincipal CustomUserDetails userDetails, @PathVariable Integer reviewId, @Valid @RequestBody ReviewUpdateRequest request) {
  return reviewService.updateReview(userDetails.getMember().getMemberId(), isAdmin(userDetails), reviewId, request);
 }

 @DeleteMapping("/{reviewId}")
 public void deleteReview(@AuthenticationPrincipal CustomUserDetails userDetails, @PathVariable Integer reviewId) {
  reviewService.deleteReview(userDetails.getMember().getMemberId(), isAdmin(userDetails), reviewId);
 }

 @PostMapping("/{reviewId}/likes")
 public ReviewLikeToggleResponse toggleLike(@PathVariable Integer reviewId, @AuthenticationPrincipal CustomUserDetails userDetails, HttpServletRequest request, HttpServletResponse response) {
 ...
 public List<MonthlyReviewStatResponse> getMonthlyReviewStatsByPeriod(@RequestParam LocalDate startDate, @RequestParam LocalDate endDate) {
 ...
 public ReviewDetailResponse getReviewDetail(@PathVariable Integer reviewId, @AuthenticationPrincipal CustomUserDetails userDetails, HttpServletRequest request) {
