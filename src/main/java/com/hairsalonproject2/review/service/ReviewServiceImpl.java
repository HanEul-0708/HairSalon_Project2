...
public class ReviewServiceImpl implements ReviewService {
 ...
 public ReviewResponse createReview(String loginMemberId, ReviewCreateRequest request) {
  Reservation reservation = reservationRepository.findById(request.getReservationId()).orElseThrow(() -> new IllegalArgumentException(RESERVATION_NOT_FOUND));
  if (!reservation.getMember().getMemberId().equals(loginMemberId)) throw new IllegalArgumentException(ONLY_OWNER_CAN_WRITE);
  if (!isReviewableReservation(reservation)) throw new IllegalArgumentException(REVIEW_AFTER_VISIT_ONLY);
  ...
  Review review = Review.builder().reservation(reservation).member(reservation.getMember()).designer(reservation.getDesigner()).rating(request.getRating()).content(request.getContent()).build();
  Review savedReview = reviewRepository.saveAndFlush(review);
  salonQueryService.refreshStatistics(savedReview.getDesigner().getSalon().getSalonId());
  return toResponse(savedReview, loginMemberId, null);
 }

 @Override
 public ReviewResponse getReview(Integer reviewId, String loginMemberId, String visitorToken) {
  Review review = reviewRepository.findById(reviewId).orElseThrow(() -> new IllegalArgumentException(REVIEW_NOT_FOUND));
 ...
 public ReviewDetailResponse getReviewDetail(Integer reviewId, String loginMemberId, String visitorToken) {
  Review review = reviewRepository.findById(reviewId).orElseThrow(() -> new IllegalArgumentException(REVIEW_NOT_FOUND));
  List<ReviewImageResponse> images = reviewImageRepository.findByReview_ReviewId(reviewId).stream().map(image -> new ReviewImageResponse(image.getImageId(), image.getImageUrl(), image.getSortOrder())).toList();
  return new ReviewDetailResponse(review.getReviewId(), review.getReservation().getReservationId(), review.getReservation().getReservationDate(), review.getReservation().getReservationTime(), review.getMember().getMemberId(), review.getMember().getName(), review.getDesigner().getDesignerId(), review.getDesigner().getName(), review.getReservation().getSalonService().getName(), review.getRating(), review.getContent(), review.getReplyContent(), review.getReplyCreatedAt(), review.getCreatedAt(), safeLikeCount(review), isLikedByCurrentUser(review.getReviewId(), loginMemberId, visitorToken), images);
 }

 @Override
 public List<ReviewResponse> getAllReviews(String loginMemberId, String visitorToken, Integer designerId, String sortBy) {
  List<Review> reviews = designerId == null ? reviewRepository.findAll() : reviewRepository.findByDesigner_DesignerId(designerId);
  return reviews.stream().map(review -> toResponse(review, loginMemberId, visitorToken)).sorted(resolveComparator(sortBy)).toList();
 }

 @Override
 public List<ReviewResponse> getReviewsByMember(String memberId, String loginMemberId, String visitorToken) {
  return reviewRepository.findByMember_MemberId(memberId).stream().map(review -> toResponse(review, loginMemberId, visitorToken)).sorted(Comparator.comparing(ReviewResponse::getCreatedAt).reversed()).toList();
 }
 ...
 public void deleteReview(String loginMemberId, boolean isAdmin, Integer reviewId) {
  ...
  salonQueryService.refreshStatistics(salonId);
 }
 ...
 public List<DesignerRankingResponse> getTop3Designers() {
  return reviewRepository.findTopDesignersByAverageRatingAndReviewCount(1L).stream().limit(3).toList();
 }
 ...
 public List<MonthlyReviewStatResponse> getMonthlyReviewStatsByPeriod(LocalDate startDate, LocalDate endDate) {
  return reviewRepository.findMonthlyReviewStatsByPeriod(startDate.atStartOfDay(), endDate.atTime(23, 59, 59));
 }
 ...
 public List<ReviewResponse> getRecentReviews() {
  return reviewRepository.findAll().stream().map(review -> toResponse(review, null, null)).sorted(Comparator.comparing(ReviewResponse::getCreatedAt).reversed()).limit(3).toList();
 }
 ...
 private ReviewLike findExistingLike(Integer reviewId, String loginMemberId, String visitorToken) {
  if (loginMemberId != null && !loginMemberId.isBlank())
   return reviewLikeRepository.findByReview_ReviewIdAndMember_MemberId(reviewId, loginMemberId).orElse(null);
  if (visitorToken != null && !visitorToken.isBlank())
   return reviewLikeRepository.findByReview_ReviewIdAndVisitorToken(reviewId, visitorToken).orElse(null);
  return null;
 }
 ...
 private ReviewLike createReviewLike(Review review, String loginMemberId, String visitorToken) {
  if (loginMemberId != null && !loginMemberId.isBlank()) {
   ...
   return ReviewLike.builder().review(review).member(member).build();
  }
  if (visitorToken == null || visitorToken.isBlank()) throw new IllegalArgumentException(VISITOR_TOKEN_REQUIRED);
  return ReviewLike.builder().review(review).visitorToken(visitorToken).build();
 }

 private ReviewResponse toResponse(Review review, String loginMemberId, String visitorToken) {
  String thumbnailImageUrl = review.getReviewImages().stream().sorted(Comparator.comparing(image -> image.getSortOrder() == null ? Integer.MAX_VALUE : image.getSortOrder())).map(image -> image.getImageUrl()).findFirst().orElse(null);
  return new ReviewResponse(review.getReviewId(), review.getReservation().getReservationId(), review.getReservation().getReservationDate(), review.getReservation().getReservationTime(), review.getMember().getMemberId(), review.getMember().getName(), review.getDesigner().getDesignerId(), review.getDesigner().getName(), review.getDesigner().getSalon().getName(), review.getReservation().getSalonService().getName(), review.getRating(), review.getContent(), review.getReplyContent(), review.getReplyCreatedAt(), review.getCreatedAt(), thumbnailImageUrl, safeLikeCount(review), isLikedByCurrentUser(review.getReviewId(), loginMemberId, visitorToken));
 }

 private Comparator<ReviewResponse> resolveComparator(String sortBy) {
  if ("rating".equalsIgnoreCase(sortBy))
   return Comparator.comparing(ReviewResponse::getRating).reversed().thenComparing(ReviewResponse::getCreatedAt, Comparator.reverseOrder());
  if ("likes".equalsIgnoreCase(sortBy))
   return Comparator.comparing(ReviewResponse::getLikeCount).reversed().thenComparing(ReviewResponse::getCreatedAt, Comparator.reverseOrder());
