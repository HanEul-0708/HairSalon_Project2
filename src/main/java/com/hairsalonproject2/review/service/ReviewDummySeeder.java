...
public class ReviewDummySeeder {
 ...
 public int seedReviewsFromCompletedReservations() {
  List<Reservation> completedReservations = reservationRepository.findByStatus(ReservationStatus.COMPLETED).stream().sorted(Comparator.comparing(Reservation::getReservationId)).toList();
  int created = 0;
  for (int index = 0; index < completedReservations.size(); index++) {
   Reservation reservation = completedReservations.get(index);
   if (reviewRepository.findByReservation_ReservationId(reservation.getReservationId()).isPresent()) continue;
   byte rating = determineRating(index, reservation);
   Review review = Review.builder().reservation(reservation).member(reservation.getMember()).designer(reservation.getDesigner()).rating(rating).content(buildContent(reservation, rating, index)).build();
   Review savedReview = reviewRepository.saveAndFlush(review);
   salonQueryService.refreshStatistics(savedReview.getDesigner().getSalon().getSalonId());
   dummyTimelineService.alignReviewTimeline(savedReview.getReviewId(), reservation.getReservationId(), reservation.getReservationDate(), reservation.getReservationTime());
   created++;
  }
  return created;
 }
 ...
 private String buildContent(Reservation reservation, byte rating, int index) {
  ...
  if (rating >= 5) return switch (index % 4) {
  ...
  if (rating == 4) return switch (index % 4) {
  ...
  if (rating == 3) return switch (index % 4) {
