...
public class ReviewImageServiceImpl implements ReviewImageService {
 ...
 public ReviewImageResponse uploadReviewImage(String loginMemberId, boolean isAdmin, Integer reviewId, MultipartFile file, Integer sortOrder) {
  ...
  // 7️⃣ DTO 반환
  return new ReviewImageResponse(savedImage.getImageId(), savedImage.getImageUrl(), savedImage.getSortOrder());
 }
 ...
 public List<ReviewImageResponse> getImagesByReview(Integer reviewId) {
  return reviewImageRepository.findByReview_ReviewId(reviewId).stream().map(image -> new ReviewImageResponse(image.getImageId(), image.getImageUrl(), image.getSortOrder())).toList();
 }
 ...
 public void deleteImage(String loginMemberId, boolean isAdmin, Integer imageId) {
  ReviewImage reviewImage = reviewImageRepository.findById(imageId).orElseThrow(() -> new IllegalArgumentException("삭제할 이미지가 없습니다."));
