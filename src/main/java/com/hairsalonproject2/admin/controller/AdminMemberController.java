...
public class AdminMemberController {
 ...
 @GetMapping
 public String memberList(@RequestParam(required = false) String keyword, @RequestParam(required = false) MemberRole role, @RequestParam(required = false) MemberStatus status, @RequestParam(defaultValue = "latest") String sort, @RequestParam(defaultValue = "0") int page, Model model) {
  Page<MemberSummaryResponse> memberPage = memberService.searchMembers(keyword, role, status, sort, page);
  ...
  model.addAttribute("seedGuide", "디자이너 " + targetDesignerCount + "명, 회원 " + targetUserCount + "명까지 생성합니다. 공통 비밀번호는 " + seedPassword + " 입니다.");
 ...
 public String deleteMember(@PathVariable String memberId, @AuthenticationPrincipal CustomUserDetails userDetails, RedirectAttributes redirectAttributes) {
  try {
 ...
 public String changeRole(@PathVariable String memberId, @RequestParam MemberRole role, @AuthenticationPrincipal CustomUserDetails userDetails, RedirectAttributes redirectAttributes) {
  try {
 ...
 public String changeStatus(@PathVariable String memberId, @RequestParam MemberStatus status, @AuthenticationPrincipal CustomUserDetails userDetails, RedirectAttributes redirectAttributes) {
  try {
