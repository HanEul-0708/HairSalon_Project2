...
public class SalonQueryService {
    ...
    public List<SalonRankResponse> recommendedTop3() {
        ...
        for (int i = 0; i < salons.size(); i++) {
            ...
            result.add(SalonRankResponse.builder().rank(i + 1).salonId(s.getSalonId()).salonName(s.getName()).averageRating(s.getAverageRating()).reviewCount(s.getReviewCount()).likeCount(s.getLikeCount()).build());
        }
        return result;
    }
    ...
    public Integer create(SalonCreateRequest request) {
        Salon salon = Salon.builder().externalId(request.getExternalId()).sourceType(request.getSourceType()).name(request.getName()).address(request.getAddress()).roadAddress(request.getRoadAddress()).phone(request.getPhone()).description(request.getDescription()).latitude(request.getLatitude()).longitude(request.getLongitude()).imageUrl(request.getImageUrl()).placeUrl(request.getPlaceUrl()).reservable(request.getReservable() != null ? request.getReservable() : Boolean.TRUE).build();
    ...
    public boolean like(Integer salonId, String memberId) {
        Salon salon = salonRepository.findById(salonId).orElseThrow(() -> new EntityNotFoundException("Salon not found: " + salonId));
        if (salonLikeRepository.findByMemberIdAndSalonSalonId(memberId, salonId).isPresent()) {
            return false;
        }
        salonLikeRepository.save(SalonLike.builder().memberId(memberId).salon(salon).build());
        salon.setLikeCount(Math.toIntExact(salonLikeRepository.countBySalonSalonId(salonId)));
        return true;
    }
    ...
    public boolean unlike(Integer salonId, String memberId) {
        Salon salon = salonRepository.findById(salonId).orElseThrow(() -> new EntityNotFoundException("Salon not found: " + salonId));
        return salonLikeRepository.findByMemberIdAndSalonSalonId(memberId, salonId).map(like -> {
    ...
    private SalonSummaryResponse toSummary(Salon salon, Double distanceKm) {
        return SalonSummaryResponse.builder().salonId(salon.getSalonId()).name(salon.getName()).address(salon.getAddress()).phone(salon.getPhone()).imageUrl(salon.getImageUrl()).averageRating(salon.getAverageRating()).reviewCount(salon.getReviewCount()).likeCount(salon.getLikeCount()).reservable(salon.getReservable()).distanceKm(distanceKm).build();
    }
    ...
    private Double resolveDistanceKm(SalonSearchRequest request, Salon salon) {
        ...
        double distance = GeoUtils.distanceKm(request.getLatitude().doubleValue(), request.getLongitude().doubleValue(), salon.getLatitude().doubleValue(), salon.getLongitude().doubleValue());
