...
public class SalonSeedDataFactory {
    ...
    public List<Designer> createDesigners(Salon salon) {
        ...
        return List.of(
                ...
                Designer.builder()
                        ...
                        .careerYears(second.careerYears())
                        .specialty(second.specialty())
                        .build(),
                Designer.builder()
                        .salon(salon)
                        .name(third.name())
                        .introduction(third.introduction())
                        .careerYears(third.careerYears())
                        .specialty(third.specialty())
                        .build()
        );
    }
    ...
    public List<SalonService> createServices(Salon salon) {
        ...
        return List.of(
                buildService(salon, "여성 커트", cutPrice(cutProfile.careerYears(), cutProfile.specialty(), true), 60, cutProfile),
                buildService(salon, "남성 커트", cutPrice(cutProfile.careerYears(), cutProfile.specialty(), false), 45, cutProfile),
                buildService(salon, "염색", colorPrice(colorProfile.careerYears(), colorProfile.specialty()), 120, colorProfile),
                buildService(salon, "펌", permPrice(permProfile.careerYears(), permProfile.specialty()), 150, permProfile),
                buildService(salon, "스타일링", stylingPrice(stylingProfile.careerYears(), stylingProfile.specialty()), 50, stylingProfile)
        );
    }
    ...
    private SalonService buildService(Salon salon, String serviceName, int price, int duration, DesignerProfile profile) {
        return SalonService.builder()
                ...
                .price(price)
                .duration(duration)
                .description(buildServiceDescription(serviceName, profile))
                .build();
    }
    ...
    private String buildServiceDescription(String serviceName, DesignerProfile profile) {
        return profile.careerYears() + "년 경력의 " + profile.specialty().getLabel()
                + " 전문 디자이너가 진행하는 " + serviceName + " 시술입니다.";
    }

    private int cutPrice(int careerYears, DesignerSpecialty specialty, boolean womens) {
        int base = womens ? 24000 : 18000;
        int specialtyBonus = specialty == DesignerSpecialty.CUT ? 12000
                : specialty == DesignerSpecialty.PERM ? 6000
                : specialty == DesignerSpecialty.STYLING ? 7000 : 4000;
        return base + (careerYears * 2000) + specialtyBonus;
    }

    private int colorPrice(int careerYears, DesignerSpecialty specialty) {
        int specialtyBonus = specialty == DesignerSpecialty.COLOR ? 18000
                : specialty == DesignerSpecialty.PERM ? 10000
                : specialty == DesignerSpecialty.STYLING ? 9000 : 8000;
        return 65000 + (careerYears * 3500) + specialtyBonus;
    }

    private int permPrice(int careerYears, DesignerSpecialty specialty) {
        int specialtyBonus = specialty == DesignerSpecialty.PERM ? 20000
                : specialty == DesignerSpecialty.CUT ? 10000
                : specialty == DesignerSpecialty.STYLING ? 12000 : 8000;
        return 80000 + (careerYears * 4000) + specialtyBonus;
    }

    private int stylingPrice(int careerYears, DesignerSpecialty specialty) {
        int specialtyBonus = specialty == DesignerSpecialty.STYLING ? 15000
                : specialty == DesignerSpecialty.CUT ? 9000
                : specialty == DesignerSpecialty.PERM ? 7000 : 6000;
