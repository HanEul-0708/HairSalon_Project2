...
import com.hairsalonproject2.dto.response.DesignerDetailResponse;
import com.hairsalonproject2.dto.response.DesignerSummaryResponse;
...
import com.hairsalonproject2.service.projection.DesignerRatingRow;
...
public class DesignerQueryService {
    ...
    public List<DesignerSummaryResponse> search(DesignerSearchRequest request) {
        ...
        return designerRepository.findAll(DesignerSpecifications.bySearch(request)).stream()
                ...
                .sorted(Comparator.comparing(DesignerSummaryResponse::getAverageRating, Comparator.nullsLast(Comparator.reverseOrder()))
                        .thenComparing(DesignerSummaryResponse::getReviewCount, Comparator.nullsLast(Comparator.reverseOrder()))
                        .thenComparing(DesignerSummaryResponse::getCareerYears, Comparator.nullsLast(Comparator.reverseOrder())))
                .toList();
    }
    ...
    public DesignerDetailResponse getDetail(Integer designerId) {
        ...
        DesignerRatingRow row = designerRepository.findDesignerRatingRows().stream()
                .filter(r -> designerId.equals(r.getDesignerId()))
                .findFirst()
                .orElse(null);

        List<SalonServiceSummaryResponse> services = salonServiceRepository.findBySalonSalonId(designer.getSalon().getSalonId())
                .stream()
