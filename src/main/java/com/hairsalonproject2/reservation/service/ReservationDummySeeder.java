...
public class ReservationDummySeeder {
    ...
    private static final List<LocalTime> STAFF_TIMES = List.of(
            LocalTime.of(10, 0),
            LocalTime.of(11, 0),
            LocalTime.of(12, 0),
            LocalTime.of(14, 0),
    ...
    public int seedReservations() {
        ...
        List<Member> members = memberRepository.findByRoleAndStatusOrderByCreatedAtAsc(
                MemberRole.USER,
                MemberStatus.ACTIVE
        );
        List<Designer> designers = designerRepository.findAll().stream()
                ...
                .toList();
        Map<Integer, List<SalonService>> servicesBySalonId = salonServiceRepository.findAll().stream()
                .sorted(Comparator.comparing(SalonService::getServiceId))
                .collect(Collectors.groupingBy(service -> service.getSalon().getSalonId()));
        ...
        for (int index = 0; index < toCreate; index++) {
            ...
            Reservation reservation = Reservation.builder()
                    ...
                    .status(status)
                    .totalPrice(calculatePrice(salonService, designer, status))
                    .paymentMethod(determinePaymentMethod(index, status))
                    .build();
            ...
            dummyTimelineService.alignReservationTimeline(
                    savedReservation.getReservationId(),
                    slotId,
                    reservationDate,
                    reservationTime,
                    latestDependencyCreatedAt(member, designer, salonService)
            );
            created++;
        }
        return created;
    ...
    private SalonService chooseService(List<SalonService> salonServices, Designer designer, int index) {
        List<SalonService> preferredServices = salonServices.stream()
                .filter(service -> matchesSpecialty(service, designer.getSpecialty()))
                .toList();
