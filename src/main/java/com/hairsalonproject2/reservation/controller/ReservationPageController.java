...
public class ReservationPageController {
 ...
 public String reservationList(@AuthenticationPrincipal CustomUserDetails userDetails, @RequestParam(defaultValue = "1") int page, Model model) {
  List<ReservationResponse> reservations = reservationService.getMyReservations(userDetails.getMember().getMemberId()).stream().sorted(Comparator.comparing(ReservationResponse::getCreatedAt).reversed()).toList();
  ...
  List<ReservationResponse> pagedReservations = totalReservations == 0 ? Collections.emptyList() : reservations.subList(fromIndex, toIndex);
  List<Integer> reservationIds = pagedReservations.stream().map(ReservationResponse::getReservationId).toList();
  Set<Integer> reviewedReservationIds = reservationIds.isEmpty() ? Collections.emptySet() : reviewRepository.findByReservation_ReservationIdIn(reservationIds).stream().map(review -> review.getReservation().getReservationId()).collect(Collectors.toSet());
  LocalDateTime now = LocalDateTime.now();
  Map<Integer, Boolean> reviewableReservationMap = pagedReservations.stream().collect(Collectors.toMap(ReservationResponse::getReservationId, reservation -> isReviewableReservation(reservation, now)));
 ...
 public String reservationCreate(@AuthenticationPrincipal CustomUserDetails userDetails, @RequestParam(required = false) Integer salonId, Model model) {
 ...
 public String reservationEdit(@AuthenticationPrincipal CustomUserDetails userDetails, @PathVariable Integer reservationId, Model model) {
  reservationService.validateReservationAccess(reservationId, userDetails.getMember().getMemberId(), userDetails.getMember().getRole().name().equals("ADMIN"));
