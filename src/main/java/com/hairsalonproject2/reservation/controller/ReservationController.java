...
public class ReservationController {
    ...
    public Map<String, Object> checkAvailability(@RequestParam Integer designerId,
                                                  @RequestParam LocalDate reservationDate,
                                                  @RequestParam LocalTime reservationTime,
                                                  @RequestParam(required = false) Integer reservationId) {
        boolean available = reservationService.isReservationAvailable(
                designerId,
                reservationDate,
                reservationTime,
                reservationId
        ...
        return Map.of(
                ...
                "message", available
                        ? "예약 가능한 시간입니다."
                        : "선택하신 날짜와 시간에는 이미 해당 디자이너 예약이 있습니다. 다른 시간을 선택해주세요."
        );
    }

    @GetMapping("/unavailable-times")
    public Map<String, Object> getUnavailableTimes(@RequestParam Integer designerId,
                                                   @RequestParam LocalDate reservationDate,
                                                   @RequestParam(required = false) Integer reservationId) {
        List<String> unavailableTimes = reservationService.getUnavailableReservationTimes(
                        designerId,
                        reservationDate,
                        reservationId
                ).stream()
        ...
        return Map.of("unavailableTimes", unavailableTimes);
    }

    @DeleteMapping("/{id}")
    public String cancelReservation(@AuthenticationPrincipal CustomUserDetails userDetails,
                                    @PathVariable Integer id) {
        validateReservationCancelAccess(userDetails, id);
    ...
    public ReservationResponse updateReservationStatus(@AuthenticationPrincipal CustomUserDetails userDetails,
        ...
        validateReservationManagementAccess(userDetails, id);
    ...
    private boolean isDesigner(CustomUserDetails userDetails) {
        return userDetails.getMember().getRole() == MemberRole.DESIGNER;
    }

    private void validateReservationCancelAccess(CustomUserDetails userDetails, Integer reservationId) {
        if (isAdmin(userDetails)) {
            return;
        }
        if (isDesigner(userDetails)) {
            reservationService.validateReservationDesignerSalonAccess(reservationId, memberId(userDetails));
            return;
        }
        validateReservationAccess(userDetails, reservationId);
    }

    private void validateReservationManagementAccess(CustomUserDetails userDetails, Integer reservationId) {
        if (isAdmin(userDetails)) {
            return;
        }
        reservationService.validateReservationDesignerSalonAccess(reservationId, memberId(userDetails));
